...
public class Settings {
    ...
    private static void CompareSettings() {
        for (String key : defaultSettingsMap.keySet()) {
            if (!settingsMap.containsKey(key) || settingsMap.get(key) == null || settingsMap.get(key).isEmpty()) {
                settingsMap.put(key, defaultSettingsMap.get(key));
                System.out.println("Added or fixed setting: " + key);
            }
        }
    }
